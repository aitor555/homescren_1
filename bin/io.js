...
function catchLR()
{
	$("#input").keyup(function(e)
	{
		if (e.which == 37 && $("#input").text().length > 0)
		{
			...
			$("#input").remove();
			$("#prompt").append("<pre id = 'input' contentEditable = 'true' tabindex='0'>" + currentText.substring(0, currentText.length-1) + "</pre>");
			focusEnd();
		...
		else if (e.which == 39)
		{
			...
			$("#input").remove();
			$("#prompt").append("<pre id = 'input' contentEditable = 'true' tabindex='0'>" + newText + "</pre>");
			focusEnd();
	...
	$("#input").keyup(function(e)
	{
		if (e.which == 38)
		{
			if (historyIndex >= 2)
			{
				historyIndex--;
				$("#input").remove();
				$("#prompt").append("<pre id = 'input' contentEditable = 'true' tabindex='0'>" + historyList[historyIndex] + "</pre>");
				$(".ppost").remove();
				focusEnd();
				setTimeout(10,resetInput());
			}
		...
		else if (e.which == 40)
		{
			...
			{
				$("#input").remove();
				$("#prompt").append("<pre id = 'input' contentEditable = 'true' tabindex='0'></pre>");
				$(".ppost").remove();
				focusEnd();
				setTimeout(10,resetInput());
			}
